 ...
 var svg_builder = {
	 ...
	 */
	 draw_line: function (Ax, Ay, Bx, By,angle=0) {
		...
		svg_builder.create_path(tab_coordinate,angle);
	},
	 ...
	 */
	 draw_path: function (wooden_plate_thickness, size, rotate_case, draw_origin_x, draw_origin_y,angle=0,nbNotch) {
		var tab_coordinate = svg_builder.draw_side(wooden_plate_thickness, size,nbNotch); 							// gets the good values to draw
	 ...
	 * @param draw_origin_y {int} is the y (ordinate) position where we start the drawing.
	 */
	 draw_path_right_left_correction: function (wooden_plate_thickness, size, rotate_case, draw_origin_x, draw_origin_y,angle=0) {
		...
		svg_builder.create_path(tab_coordinate.join(' '),angle);
	},
	 ...
	 */
	 create_path: function (tab_coordinate, angle) {
	 ...
	 */
	 draw_side: function (wooden_plate_thickness, size,nbNotch) {
	 	var inner_size = size - ( wooden_plate_thickness * 2 );
	 	var coordinate_inner_side = svg_builder.draw_inner_side(wooden_plate_thickness, inner_size,nbNotch);
	 ...
	 */
	 draw_inner_side: function (wooden_plate_thickness, size, nbNotch) {

	 	/*if(nbNotch == 0){
	 		var number_notch = svg_builder.get_number_notch(wooden_plate_thickness, size);
	 	}
	 	else{
	 		var number_notch = nbNotch;
	 	}*/
	 	var number_notch = nbNotch || svg_builder.get_number_notch(wooden_plate_thickness, size);
 ...
 var Box_without_top = {
	 ...
	 */
	 draw_single_part: function (number_part, origin_x, origin_y, wooden_plate_thickness, width_box, depth_box, height_box, bool_top, bool_right, bool_bot, bool_left) {
	 	if(number_part == 1) {
	 		...
	 		if(bool_right) 	svg_builder.draw_path(wooden_plate_thickness, height_box, 4, origin_x + width_box, origin_y, 0, 3);
	 		if(bool_bot) 	svg_builder.draw_path(wooden_plate_thickness, width_box, 3, origin_x + width_box, origin_y + height_box);
	 		if(bool_left) 	svg_builder.draw_path(wooden_plate_thickness, height_box, 6, origin_x, origin_y + height_box);
	 		svg_builder.define_box_width_and_length(width_box + 10, height_box + 10);
	...
	var Toolbox = {
		...
		draw_base_side: function(origin_x, origin_y, nbNotch, height_box, width_box, nose, depth_box){
			...
			sizeBetweenBigNotch = (width_box - Math.tan(45)*((height_box-nose)/2) - (NOTCH_SIZE * 2)*nbNotch) / (nbNotch+1);
		...
		draw_top: function(origin_x, origin_y, nbNotch, height_box, width_box, nose, rotate){
			...
			var l_origin_x = origin_x + Math.tan(45)*((height_box-nose)/2);
		...
		draw_single_part: function (number_part, origin_x, origin_y, wooden_plate_thickness, width_box, depth_box, height_box, bool_top, bool_right, bool_bot, bool_left, nose) {
			//svg_builder.draw_line(origin_x, origin_y, Math.tan(45)*((height_box-nose)/2), -(height_box-nose)/2);
			/*
			...
			svg_builder.draw_line(origin_x + (width_box - Math.tan(45)*((height_box-nose)/2)),origin_y,0,depth_box);*/

			nbNotch = Math.round((width_box - Math.tan(45)*((height_box-nose)/2))/70);
			hypothenuse = Math.sqrt(((height_box-nose)/2)*((height_box-nose)/2) +  (Math.tan(45)*(height_box-nose)/2)*(Math.tan(45)*(height_box-nose)/2));
			if(number_part == 1) {
				if(bool_top)	Toolbox.draw_base_side(origin_x, origin_y, nbNotch, height_box, width_box-wooden_path, nose, depth_box);	
				if(bool_right) 	svg_builder.draw_line(origin_x + (width_box - Math.tan(45)*((height_box-nose)/2)),origin_y,0,depth_box);
				if(bool_bot) 	Toolbox.draw_base_side2(origin_x, origin_y, nbNotch, height_box, width_box, nose, depth_box);
				if(bool_left)	svg_builder.draw_line(origin_x,origin_y,0,depth_box); 	
				svg_builder.define_box_width_and_length(width_box + 10, height_box + 10);
			} else if(number_part == 6) {
				svg_builder.draw_line(origin_x, origin_y, Math.tan(45)*((height_box-nose)/2), -(height_box-nose)/2);
				if(bool_top)	Toolbox.draw_top(origin_x, origin_y, nbNotch, height_box, width_box, nose,3);
				if(bool_right) 	svg_builder.draw_path(wooden_plate_thickness, height_box, 4, origin_x + width_box, origin_y -(height_box-nose)/2);
				if(bool_bot) 	Toolbox.draw_bot(origin_x + width_box, origin_y + height_box, nbNotch, height_box, width_box, nose,1);
				if(bool_left) 	svg_builder.draw_path(wooden_plate_thickness, nose, 5, origin_x, origin_y);
				svg_builder.draw_line(origin_x, origin_y+nose, Math.tan(45)*((height_box-nose)/2), (height_box-nose)/2);
				svg_builder.define_box_width_and_length(width_box + 10, depth_box + 10);
			} else if(number_part == 2) {
				svg_builder.draw_line(origin_x, origin_y, Math.tan(45)*((height_box-nose)/2), -(height_box-nose)/2);
				if(bool_top)	Toolbox.draw_top(origin_x, origin_y, nbNotch, height_box, width_box, nose,0);
				if(bool_right) 	svg_builder.draw_path(wooden_plate_thickness, height_box, 4, origin_x + width_box, origin_y -(height_box-nose)/2);
				...
				if(bool_bot) 	/*svg_builder.draw_line(origin_x, origin_y + nose, depth_box,0);*/svg_builder.draw_path(wooden_plate_thickness, nose, 2, origin_x+nose, origin_y+depth_box);
				...
				if(bool_bot) 	/*svg_builder.draw_line(origin_x, origin_y + hypothenuse, depth_box,0);*/svg_builder.draw_path(wooden_plate_thickness, hypothenuse, 2, origin_x + hypothenuse, origin_y+depth_box);
			...
			//svg_builder.draw_line(origin_x, origin_y, Math.tan(45)*((height_box-nose)/2), -(height_box-nose)/2);
			//svg_builder.draw_path(wooden_plate_thickness, width_box-Math.tan(45)*((height_box-nose)/2), 1, origin_x + Math.tan(45)*((height_box-nose)/2), origin_y -(height_box-nose)/2,0,3);
			//svg_builder.draw_path(wooden_plate_thickness, height_box, 4, origin_x + width_box, origin_y -(height_box-nose)/2);
		...
		economize_laser_and_wood_one_box: function (origin_x, origin_y, wooden_plate_thickness, width_box, depth_box, height_box, nose) {
			hypothenuse = Math.sqrt(((height_box-nose)/2)*((height_box-nose)/2) +  (Math.tan(45)*(height_box-nose)/2)*(Math.tan(45)*(height_box-nose)/2));
			Toolbox.draw_single_part(6, origin_x, origin_y, wooden_plate_thickness, width_box, depth_box, height_box, true, true, true, true, nose);
			// Toolbox.draw_single_part(1, origin_x +  (Math.tan(45)*(height_box-nose)/2), origin_y + (height_box-nose)/2 + nose, wooden_plate_thickness, width_box, depth_box, height_box, false, true, true, true, nose);
			// Toolbox.draw_single_part(6, origin_x, origin_y + height_box + depth_box, wooden_plate_thickness, width_box, depth_box, height_box, false, true, true, true, nose);
			// Toolbox.draw_single_part(4, origin_x + hypothenuse, origin_y + ((height_box-nose)/2) + nose + height_box + depth_box + wooden_plate_thickness, wooden_plate_thickness, width_box, depth_box, height_box, true, true, true, false, nose);
			...
			// Toolbox.draw_single_part(5, origin_x, origin_y + ((height_box-nose)/2) + nose + height_box + depth_box + wooden_plate_thickness, wooden_plate_thickness, width_box, depth_box, height_box, true, true, true, true, nose);
			svg_builder.define_box_width_and_length(width_box + depth_box + 10, height_box * 2 + depth_box + 10);
		},
...
function test2(){
	...
	nose = 30;
	Toolbox.economize_laser_and_wood_one_box(wooden_plate_thickness+40, wooden_plate_thickness+40, wooden_plate_thickness, width_box, depth_box, height_box,nose);
